...
#analyze resume depends on current user ke upr--->fastapi auto calls this function first and take token from request header--->verifys user id--->inject in endpoint
...
async def analyze_resume(
    ...
    user_id: Optional[str] = Depends(get_current_user),    #DEPENDENCY INJECTION 
):
    warnings: List[str] = []
    ...
    response = AnalysisResponse(
        ...
        # 🆕 COLLEGE PROJECT REQUIREMENT: MAP CO-PILOT FIELDS TO RESPONSE SCHEMA
        fit_analysis=result.get('fit_analysis', {"requirements_met": [], "requirements_lacks": [], "strategic_emphasis": ""}),
        rewritten_resume=result.get('rewritten_resume', "No resume optimization generated."),
        cover_letter=result.get('cover_letter', "No cover letter generated."),
    ...
    try:
        ...
        # Only save if user_id exists (i.e., if the user is logged in)
        if user_id:
            await save_analysis(user_id, filename, result)
        else:
            logger.info("Guest user detected: skipping database save.")
    except Exception as exc:
...
#An endpoint health to check the current status of the file
@router.get('/health')
...
async def generate_history_pdf(
    ...
    # Safely extract the target analysis item
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate PDF for history: {e}")