...
# 🧠 COLLEGE PROJECT CONFIGURATION: Your custom fine-tuned model path
FINETUNED_MODEL_PATH = "backend/saved_models/finetuned-bert"

def calculate_semantic_similarity(
    resume_text: str, jd_text: str, embedder: SentenceTransformer = None
) -> float:
    """
    Calculates semantic similarity using your custom fine-tuned BERT model parameters.
    """
    # Fallback: If no embedder was passed from routes/scorer, load your custom fine-tuned weights locally
    if embedder is None:
...
def compare_resume_with_jd(
    ...
    """
    Core pipeline comparison engine executing against your fine-tuned weights.
    """
    # Ensure our custom model path fallback handles the session lifecycle
    if embedder is None:
        embedder = SentenceTransformer(FINETUNED_MODEL_PATH)
