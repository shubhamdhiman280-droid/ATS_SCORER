#RECAP TILL THIS FILE:-->USER UPLOAD--->VALIDATE--->PARSE---->DATA(RAW, UNSTRUCTURED)----->GROQ LLM--->STRUCTURE(JSON)---->TRAIN,FINETUNE(BERT, ON DATASET)---->JD MATCH---->ATS SCORE MATCHING ALL THINGS THAT IS IN RESUME AND ASSIGN SCORE---------->recommendation engine give feedbacks which issue to fix first and last----------->recommend and rank issues to fix(critical,high,medium,low)(if all errors shown together user might get suprized)----------->report generator.py(we will create some predefined templates stored in templates)
...
#Location/privacy detection and how it effects the ATS Score
...
#Skill validation: to validate the proof of work mentioned in in resume
