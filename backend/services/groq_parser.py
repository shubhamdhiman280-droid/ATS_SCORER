...
logger=logging.getLogger('ats_resume_scorer') #by loggere we can classify into 3 types


GROQ_MODEL='llama-3.3-70b-versatile'

_client=None #private variable




def _get_client()->Groq:   #o/p expected from groq object
    global _client
    if _client is None:
...
#Singleton pattern
        ...
        _client=Groq(api_key=api_key) #Getting key from .env
    return _client
...
def _try_parse_json(text: str) -> dict | None:
    ...
    cleaned = text.strip()
    if cleaned.startswith("```"): #where we strip off ''' form our json so no further problem
    ...
    #Giving second strict prompt so things are properly done
def parse_resume(raw_text: str)->Dict:
    ...
    if result is None:
        return _validate_resume_result(result) #check things which we get after parsing resume actually exists their is Rsume or not
...
def _validate_jd_result(result: dict) -> dict: #ensures dict is comming
...
#TO VALIDATE THE EXPERIENCE IN RESUME
####################################################
...
#TO VALIDATE PROJECTS IN RESUME
