...
def display_results_dashboard(analysis: Dict[str, Any]) -> None:
    ...
    # 🆕 1. Inject College Re-Branding Header right at the top
    st.markdown("## 📊 Co-Pilot Target Analytics Dashboard")
    st.caption("Review your core structural performance rating alongside parallel alignment assets below.")
    st.markdown("---")

    # Render your existing high-quality widgets
    ...
    # 🆕 2. Append an overview card to prompt the user about their generation assets 
    # (Since scorer.py handles rendering the heavy tab view right under this dashboard)
    if "fit_analysis" in analysis:
        st.success("💡 **Co-Pilot Assets Compiled!** Scroll down directly below this dashboard to access your generated Fit Analysis, Side-by-Side Diff Optimizer, Cover Letter, and Interview Question Packs!")